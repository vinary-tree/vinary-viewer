...
| 16 | **In-pane directory browser** | Available now | Opening a folder lists its children in the preview pane as a detailed list, with live refresh. | Open a directory: `vv ~/docs`, a folder in the file-tree, or a directory link; open an entry (single-click on Linux, double-click on Windows/macOS, or `Enter` / `Alt+↓`), `Ctrl+click` or middle-click for a background tab. | [16-directory-browser.md](16-directory-browser.md) |
| 17 | **Breadcrumb + up/down navigation** | Available now | A Ctrl-hover breadcrumb URI bar, `Alt+↑` to the parent folder, `Alt+↓` to open the highlight, and a persisted trail so `Alt+↑` then `Alt+↓` returns to the last-opened file. | Hold `Ctrl` over the URI bar to get clickable path segments; use `Alt+↑` / `Alt+↓` to traverse folders. | [17-breadcrumb-and-up-down-navigation.md](17-breadcrumb-and-up-down-navigation.md) |
| 18 | **Address-bar completion** | Available now | Fish-style path auto-completion (inline ghost + ambiguous-only dropdown) for local files, and browser-history completion for web URLs. | Focus the URI bar (`Ctrl+L`); `Tab` completes the common prefix, `→` accepts the ghost, `↑`/`↓` pick, `Enter` opens. | [18-address-bar-completion.md](18-address-bar-completion.md) |
| 19 | **Web-view keyboard, Copy & cookies** | Available now | The in-app web view has vim/standard scroll keys + `f` link hints, a right-click Copy menu, native selection, and a persistent session so logins survive restarts; local `.html` files open here as live pages, and any overlay freezes the page **instantly** (a pre-cached pushed snapshot) instead of blanking it. | Open an `http(s)` link or a local `.html`; scroll with `j`/`k`/Space/arrows, `f` for hints, right-click to Copy. | [19-web-view-keyboard-and-copy.md](19-web-view-keyboard-and-copy.md) |
| 20 | **Ad & tracker blocking** | Available now | Native EasyList/uBO blocking on the web session (no extension), auto-updated and toggleable, with inline **Update filter lists** feedback. | `Settings ▸ Extensions ▸ Ad blocking`. | [20-ad-blocking.md](20-ad-blocking.md) |
| 21 | **Browser extensions** | Available now | Install ad-blocker-class and lightweight Chrome extensions from the Web Store, with auto-update, a toolbar, and popups; an inert chrome.* polyfill keeps popup/options pages from crashing on APIs Electron lacks, while heavy MV3 password-manager workers remain a documented Electron gap. | `Settings ▸ Extensions`; paste a Web Store URL/ID and Install. | [21-browser-extensions.md](21-browser-extensions.md) |
| 22 | **Zoom & Fit** | Available now | One context-aware zoom: a bottom zoom bar (− / editable % / presets / +) plus a **View ▸ Fit** submenu, targeting the PDF scale, the web page, or the app window depending on the active tab. | Use the bottom zoom bar, `Ctrl` `+` / `-` / `0`, or **View ▸ Fit**. | [22-zoom-and-fit.md](22-zoom-and-fit.md) |
| 23 | **Native password-manager bridge** | Available now | Fill and save web-view logins through main-process provider CLIs for 1Password, LastPass, optional Bitwarden/Proton Pass, and restricted JSON adapters. | Click the key icon on an `http(s)` tab, choose a matching login, or accept a save prompt after submitting a login form. | [23-password-manager-bridge.md](23-password-manager-bridge.md) |
| 24 | **Dialogs & modals** | Available now | A shared modal shell gives every dialog a defined border + theme-aware elevation, Esc / ✕ / backdrop close, autofocus, a focus trap, and true modality; the palette and menus share the same elevation tokens. Error-view text is selectable and copyable. | Open `Settings ▸ Preferences / Extensions / Passwords`, `Help ▸ About / Command Palette`; close with `Esc`, the `✕`, or a backdrop click. | [24-dialogs-and-modals.md](24-dialogs-and-modals.md) |
...
| 29 | **Remote files over SSH** | Available now | Open remote files and directories over `ssh://` / `sftp://` through the same renderers, streaming, and paging. A compatible target daemon adds authenticated push refresh and expansion-scoped Files trees; opt-in polling is the no-daemon content fallback. SSH secrets stay main-side. | Type or pass `ssh://user@host/path`; authenticate at the prompt. | [29-remote-files-over-ssh.md](29-remote-files-over-ssh.md) |
...
| 31 | **Diff project adoption** | Available now | A diff document puts the repository it describes into Files — a piped diff adopts the invoking cwd's repo, an on-disk patch the repo owning its resolved targets — with the diff pinned inside as an italic attached row ("(piped diff)" for stdin: transient with the tab; persistent for files). | `git diff \| vv -t diff`, or open a `.patch` stored outside its checkout; click the pinned row like any file. | [31-diff-project-adoption.md](31-diff-project-adoption.md) |
| 32 | **Commits tab** | Available now | A fourth sidebar panel (reached from the vertical icon rail, ADR-0041) shows the active document's repository history: a paged git log with a mini lane-graph rail, ref chips, lazily GFM-rendered messages, a type-to-filter branch/tag combo, and live refresh on commit/fetch/branch-switch. Click a commit for its diff vs parent; Ctrl+click two + "Diff selected", or type `A..B` / `A...B` / any rev — all opening as ordinary diff tabs with rev-exact Split enrichment. Strictly read-only git. | Open the **Commits** sidebar tab in any git project; click a commit, or type `HEAD~3..HEAD` in the range box. | [32-commits-tab.md](32-commits-tab.md) |
| 33 | **Commit Graph** | Available now | The full-pane, GitLens-style history document ("Commits · \<repo\>"): a wide lane graph (12 lanes + `»` overflow), ref badges (`HEAD →`/tags/remotes), author/date columns, a complete keyboard model (arrows/Shift/Ctrl, PgUp/PgDn, Home/End, Space, Enter), auto-paging by scroll, and live refresh — sharing one store and one hash-keyed selection with the Commits tab, so the two surfaces mirror each other. | Palette → **Open commit graph**; or right-click a Files project header → **Open Commit Graph**; or the **Graph** pill in the Commits panel. | [33-commit-graph.md](33-commit-graph.md) |
| 34 | **Git blame & file history** | Available now | A per-line blame gutter on any source view — "author · age" chips, italic **Uncommitted** for dirty lines, hover for the commit, click to open its diff, live-refresh-aware — plus focused histories shown in both Commits surfaces: per-file (`--follow`, crossing renames) and per-line-range (`git log -L`), HEAD-pinned, with a dismissible chip back to the branch log. | `Ctrl+Shift+G` (or palette → **Toggle git blame**) over a source file; right-click a file/tab → **File History**; select lines in a source view → right-click → **Line Range History**. | [34-git-blame-and-file-history.md](34-git-blame-and-file-history.md) |
...
- [`seq-commits-log.puml`](../diagrams/seq-commits-log.puml) + [`seq-git-open-diff.puml`](../diagrams/seq-git-open-diff.puml) — the Commits panel's log delivery / live refresh, and a commit diff from row click through the spilled document to rev-aware Split enrichment (feature 32).
- [`component-git-integration.puml`](../diagrams/component-git-integration.puml) — the whole git integration in one picture: the main-side data layer and `.git` watcher, the `vv:git-*` channels, the shared `[:ui :commits]` / `[:ui :blame]` stores with the watch-owner union, and the four consuming surfaces — Commits panel, Commit Graph document, blame gutter, diff documents (features 32–34 + 28).
