...
| 21 | **Browser extensions** | Available now | Install password managers + ad-blocker-class Chrome extensions from the Web Store, with auto-update, a toolbar, and popups; an inert chrome.* polyfill keeps popup/options pages from crashing on APIs Electron lacks (background service workers remain a documented gap). | `Settings ▸ Extensions`; paste a Web Store URL/ID and Install. | [21-browser-extensions.md](21-browser-extensions.md) |
