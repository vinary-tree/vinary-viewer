...
| 16 | **In-pane directory browser** | Available now | Opening a folder lists its children in the preview pane as a detailed list, with live refresh. | Open a directory: `vv ~/docs`, a folder in the file-tree, or a directory link; open an entry (single-click on Linux, double-click on Windows/macOS, or `Enter` / `Alt+↓`), `Ctrl+click` for a new tab. | [16-directory-browser.md](16-directory-browser.md) |
