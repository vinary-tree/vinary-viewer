(ns vinary.ir.frontend.log-stream
  "Streaming log front-end: line batches → :record IR blocks, emitted INCREMENTALLY with bounded memory. A
   ...
   config. Pure + DOM-free (a StreamParser); the DOM append happens later in vinary.stream.sink."
...
(defn- record-node
  "Build a :record IR block from its lines with a stable anchor id (threaded `seen` for rehype-slug-style
   uniqueness). Each line lowers to a div.vv-log-line; the record to a div.vv-log-record carrying the id."
  [lines seen]
  (let [kids (mapv line-node lines)
        base (node/node :record kids)
        id   (meta/anchor-id base seen)]
    (node/node :record kids {:tag "div" :attrs {"className" ["vv-log-record"] "id" id}
                             :id id :role :log-record})))

(defrecord LogStreamParser [frontier open seen]
  proto/StreamParser
  (feed [_ lines]
    (loop [ls (seq lines), fr frontier, open open, blocks []]
      (if (nil? ls)
        {:parser (LogStreamParser. fr open seen) :blocks blocks}   ; ctor form: ->LogStreamParser isn't declared yet inside its own methods
        (let [line      (first ls)
              ...
              [open blocks] (if boundary?
                              [[line] (conj blocks (record-node open seen))]
                              [(conj open line) blocks])]
          (recur (next ls) (glog/advance-net fr (net-brace line)) open blocks)))))
  (finish [_] {:blocks (if (seq open) [(record-node open seen)] [])}))

(defn parser
  "A fresh streaming log parser (empty open record, brace depth 0, a private id-occurrence counter)."
  []
  (->LogStreamParser (glog/initial-frontier) [] (atom {})))
