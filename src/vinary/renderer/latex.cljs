(ns vinary.renderer.latex
  "LaTeX → HTML string, the reusable front-end shared by standalone `.tex` documents (renderer.markdown-pipeline/
   ...
   and then expanded — so user macros that expand into supported constructs render correctly. The spent
   definitions are dropped (they carry no visible output). Math is preserved verbatim as `span.inline-math` /
   `div.display-math`; the caller's `tex-normalize` pass remaps those to `code.math-inline` /
   `pre>code.math-display` so the existing MathJax pass (renderer.math/render-html-math) typesets them — even
   `align`/`equation` environments survive intact into MathJax."
  (:require ["@unified-latex/unified-latex-util-parse"  :refer [parse getParser]]
            ...
            ["@unified-latex/unified-latex-util-macros"  :refer [listNewcommands expandMacros]]
...
(defn latex->html
  ...
  ([source opts]
   (let [inline?  (:inline? opts)
         ...
         src      (str source)]
     (try
       (let [;; Harvest \newcommand specs from the body AND (separately) the preamble, then combine — so macros
             ;; defined in an Org #+LATEX_HEADER preamble expand in the body WITHOUT rendering the preamble's
             ;; other content (\usepackage, \setmainfont, \backgroundsetup{…\includegraphics{logo.pdf}}), which
             ;; would inject spurious output. Only the body `src` is ever parsed for rendering.
             pre-specs  (if (seq preamble) (listNewcommands (parse (str preamble))) #js [])
             body-ast   (parse src)
             specs      (.concat pre-specs (listNewcommands body-ast))
             ast        (if (pos? (.-length specs))
                          (.parse (getParser #js {:macros (macro-signature-record specs)}) src)
                          body-ast)]
         (expandMacros ast (.map specs (fn [^js s] #js {:name (.-name s) :body (.-body s)})))
