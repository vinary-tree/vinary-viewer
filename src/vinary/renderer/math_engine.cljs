(ns vinary.renderer.math-engine
  "The heavy MathJax TeX→SVG engine — the RENDERER-ONLY (+ :node-test) half of the old renderer.math, split out
   so the @mathjax/src source + the Latin-Modern MathJax Modern font (the single biggest dependency, ~3-5 MB)
   live in a lazily-loaded build module (`:math-engine`, depends-on `:main`) instead of the renderer boot bundle
   AND instead of the :cli/:tui node bundles (which never render SVG math to the terminal). The renderer reaches
   this ns ONLY through the vinary.renderer.mathjax-lazy facade (shadow.lazy/loadable — a bare js* ref, NOT a
   :require edge), so it code-splits; renderer.core schedules an idle preload (mathjax-lazy/ensure! + install the
   stylesheet just after first paint) so pool windows warm the engine off the critical path. :node-test requires
   this ns DIRECTLY (eager) so the math tests still exercise the real engine.

   The PURE, mathjax-FREE helpers (strip-math-fence, delimit-tex, tex-error?, tex-block-math?, error-html) stay in
   vinary.renderer.math — shared eagerly by the pipeline / :cli / :tui / :node — and this ns requires that ns for
   the two pure helpers render-tex-blocks needs (tex-block-math?, tex-error?).
  ...
  (:require [clojure.string :as str]
            ;; the PURE, mathjax-free helpers this engine composes on: tex-block-math? (should this
            ;; #+BEGIN_EXPORT latex body even be ATTEMPTED as math?) and tex-error? (did MathJax emit an error
            ;; node?). They stay in renderer.math so the pipeline/:cli/:tui/:node bundles keep them without the
            ;; engine.
...
(defn install-stylesheet!
  "Insert `engine-stylesheet` into the live document once, as <style id=\"vv-mathjax-style\"> appended to <head>.
   ...
   builds. Appended AFTER app.css, but every app.css math rule is more specific than MathJax's, so the app still
   wins each tie."
  []
...
;; `data-mjx-error="Unknown environment 'center'"`. tex-error? (pure, in renderer.math) checks for both, plus a
;; real throw (a font "retry"); tex-block-math? (pure, in renderer.math) decides whether to even attempt it.
    ...
    (let [parser    (js/DOMParser.)
      ...
      (dotimes [i (.-length node-list)]
        (let [^js code  (.item node-list i)
              source    (str/trim (.-textContent code))
              rendered  (when (math/tex-block-math? source)
                          (try (let [svg (render-tex source true)]
                                 (when-not (math/tex-error? svg) svg))
                               (catch :default _ nil)))]
          (if rendered
...
;; The renderer.mathjax-lazy facade loads this module lazily and passes calls through these exported fns (string
;; keys so :simple/:advanced never rename them). Keep in sync with the pass-throughs in renderer.mathjax-lazy.
;; render-tex / engine-font-name / engine-stylesheet are exported for :node-test (which requires this ns directly)
;; and diagnostics; render-html-math / render-tex-blocks / install-stylesheet! are the facade's pass-throughs.
(def ^:export exports
  #js {"render-html-math"   render-html-math
       "render-tex-blocks"  render-tex-blocks
       "install-stylesheet!" install-stylesheet!
       "render-tex"         render-tex
       "engine-font-name"   engine-font-name
       "engine-stylesheet"  engine-stylesheet})
