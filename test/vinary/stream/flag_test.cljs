...
(deftest enabled-requires-flag-kind-and-size
  (testing "off unless flag on AND streamable AND size ≥ threshold"
    ...
    (is (flag/enabled? "markdown" 300000 true) "markdown streams above 256 KiB")
    (is (not (flag/enabled? "markdown" 1000 true)) "a small markdown doc stays batch")
    (is (flag/enabled? "pdf" 0 true) "pdf threshold is 0 (streams whenever the flag is on)")))
